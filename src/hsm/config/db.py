...
DB_USERNAME = os.getenv('DB_USERNAME', default="usuario_hsm")
DB_PASSWORD = os.getenv('DB_PASSWORD', default="password_hsm")
...
def database_connection(config, basedir=os.path.abspath(os.path.dirname(__file__))) -> str:
    ...
    if config.get('TESTING', False) == True:
        ...
        return f'mysql+pymysql://{DB_USERNAME}:{DB_PASSWORD}@{DB_HOSTNAME}/hsm'
