...
def crear_evento(dato):
    try:
        ...
        comando = CrearAnonimizacion(token_dto.id, token_dto.id_solicitud, token_dto.id_paciente, token_dto.token_anonimo, token_dto.fecha_creacion)
...
def suscribirse_a_comandos(app=None):
    ...
    try:
        cliente = pulsar.Client(f'pulsar://{utils.broker_host()}:6650')
        consumidor = cliente.subscribe('AnonimizacionCreada', consumer_type=_pulsar.ConsumerType.Shared, subscription_name='anonimizacion-sub-comandos', schema=AvroSchema(ComandoCrearToken))
        ...
        while True:
            ...
            #crear_evento(datos) # esto hay que verlo
